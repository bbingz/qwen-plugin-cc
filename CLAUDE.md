...
Claude Code plugin wrapping Qwen Code CLI(独立 git 仓库)。**v0.1.1 已发布**:
- GitHub: https://github.com/bbingz/qwen-plugin-cc(public)
- Tags: v0.1.0, v0.1.1
- 97 unit + 3 integ tests 全绿
...
- **4-way review archives**:`doc/review-v010-{claude,codex,gemini,qwen}.md`(v0.2 backlog 来源)
- **Companion 核心**:`plugins/qwen/scripts/qwen-companion.mjs` + `plugins/qwen/scripts/lib/qwen.mjs`
- **bg job finalize**:`plugins/qwen/scripts/lib/job-lifecycle.mjs::refreshJobLiveness`
...
见 `~/.claude/projects/-Users-bing--Code--qwen-plugin-cc/memory/pending-after-compact.md`。优先级:correctness(retry session / tryLocalRepair truncation / PID 复用)> DX(ajv / bg stderr 透传)> docs sync(spec §3.3/§4.6、plan.md Phase 3-5)。
