...
- [ ] **Step 1: 写脚本(复用 case-02 的过期技巧,但只抓 stream-json 原文存档)**
...
- [ ] **Step 1: 生成 >200KB 的 fixture diff**
...
- [ ] **Step 3: 写 `plugins/qwen/.claude-plugin/plugin.json`**
...
- [ ] **Step 4: 写 `plugins/qwen/CHANGELOG.md`**
...
- [ ] **Step 4: scan 有没有 `gemini` 字样(应该没有)**
...
- [ ] **Step 1: 写测试(TDD;覆盖 spec §4.3 + §6.3 qwen-proxy 清单)**
...
- [ ] **Step 3: 实现 buildSpawnEnv(严格按 spec §4.3)**
...
- [ ] **Step 1: grep gemini 源码,列出 job-control.mjs 调用的外部符号**
...
- [ ] **Step 2: grep gemini job-control.mjs 的 import 段**
...
- [ ] **Step 3: 写 `docs/superpowers/plans/_phase2-dependencies.md`**
...
- [ ] **Step 3: 写测试(spec §6.3 state.test.mjs 清单)**
...
- [ ] **Step 3: 在 qwen.mjs 实现(严格按 spec §5.1)**
...
- [ ] **Step 3: 实现 parseStreamEvents(纯离线解析;边解析边判错在 Task 2.8 streaming 版里)**
...
- [ ] **Step 1: 实现 buildQwenArgs(参数装配纯函数,先做易测的)**
...
- [ ] **Step 4: 实现 spawnQwenProcess(fg/bg 分野 + detached + unref)**
...
- [ ] **Step 5: 真机烟囱验 spawnQwenProcess(background 路径)**
...
- [ ] **Step 2: 写测试(用 fake child,验 SIGTERM 被调用)**
...
- [ ] **Step 1: 写 runTaskResumeCandidate**
...
- [ ] **Step 3: 写 prompt-blocks.md(复用 codex 的 reference 结构)**
...
- [ ] **Step 4: 写 qwen-prompt-recipes.md**
...
- [ ] **Step 6: 手测 T11 撤 token 后 rescue**
...
- [ ] **Step 1: 实现(直接写,后续 reviewWithRetry 会单测整体路径)**
...
- [ ] **Step 1: 写测试(用 fake runQwen 注入不同轮次输出)**
...
- [ ] **Step 1: 在 companion 里加 review 子命令**
...
- [ ] **Step 4: 手测 review(需要在有 diff 的仓库里)**
...
- [ ] **Step 1: 写 review.md(对齐 codex frontmatter)**
...
- [ ] **Step 2: 写 adversarial-review.md**
...
- [ ] **Step 1: 在一个有 diff 的仓库里跑 20 次 review**
...
- [ ] **Step 1: 添加 v0.1.0 release 条目到根 CHANGELOG**
...
- [ ] **Step 2: 更新 `plugins/qwen/CHANGELOG.md`**
